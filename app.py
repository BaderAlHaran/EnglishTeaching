...
try:
    from grammar_engine import correct_long_text
    _GRAMMAR_ENGINE_IMPORT_ERROR = None
except Exception as _exc:
    correct_long_text = None
    _GRAMMAR_ENGINE_IMPORT_ERROR = str(_exc)
...
def _run_local_analysis(text, progress_cb=None, timeout_seconds=20, start_time=None):
    ...
    if correct_long_text is None:
        err = "Grammar engine unavailable. Please check server configuration."
        if _GRAMMAR_ENGINE_IMPORT_ERROR:
            err = f"{err} ({_GRAMMAR_ENGINE_IMPORT_ERROR})"
        return None, err, None
    ...
    if progress_cb:
        progress_cb(10, "Running grammar model...")

    try:
        corrected_text = correct_long_text(text)
    except Exception as exc:
        return None, str(exc).strip() or "Grammar model failed unexpectedly.", None

    if progress_cb:
        progress_cb(70, "Computing highlights...")

    word_re = re.compile(r"[A-Za-z0-9]+(?:'[A-Za-z0-9]+)?")

    def _tokenize_with_spans(source_text):
        return [{'token': m.group(0), 'start': m.start(), 'end': m.end()} for m in word_re.finditer(source_text or '')]

    original_tokens = _tokenize_with_spans(text)
    corrected_tokens = _tokenize_with_spans(corrected_text)
    original_words = [t['token'] for t in original_tokens]
    corrected_words = [t['token'] for t in corrected_tokens]
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if tag == 'equal':
            continue
        if i1 == i2:
            # insertion has no original span to highlight
            continue
        start = original_tokens[i1]['start']
        end = original_tokens[i2 - 1]['end']
        suggestion = ''
        if j1 != j2:
            suggestion = corrected_text[corrected_tokens[j1]['start']:corrected_tokens[j2 - 1]['end']]
        original_segment = text[start:end]
        message = "Suggested correction."
        suggestions = []
        if j1 == j2:
            message = "Remove this word."
        elif suggestion and suggestion.strip() and suggestion.strip() != original_segment.strip():
    ...
    if timeout_seconds and (time.time() - start_time) > timeout_seconds:
        warning = "Returned results with limited post-processing due to time limits."
